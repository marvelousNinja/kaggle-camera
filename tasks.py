...
from camera.feature_extraction import inter_channel
from camera.feature_extraction import intra_channel
from camera.feature_extraction import gram_matrix
from itertools import islice
from multiprocessing import Pool

@task
def generate_test(ctx):
    data_dir = os.environ['DATA_DIR']
    ...
    test_data = list()
    for image_id, label, transform_name, image in tqdm(image_generator(test, crop_generator=center_crop, crop_size=crop_size), total=len(test)):
...
def generate_datasets(ctx):
    ...
    i = 0
    train_data = list()
    for image_id, label, transform_name, image in tqdm(parallel_image_generator(Pool(), train, crop_generator=sequential_crop, crop_size=crop_size, crop_limit=15)):
        features = np.concatenate([
            inter_channel(image),
            [int(transform_name != 'unalt'), label]
        ])
