...
def process_image(quantization, threshold, img):
    ...
    # denoised_image = denoise_nl_means(grayscale_image)
    ...
    diff = grayscale_image.astype(np.int) - denoised_image.astype(np.int)
    diff //= quantization
    ...
    horizontal_patterns = horizontal_patterns.reshape(-1)
    horizontal_patterns = horizontal_patterns[:(dim ** 4) // 2]
    vertical_patterns = np.zeros((dim, dim, dim, dim), dtype=np.int)
    np.add.at(vertical_patterns, [diff[::4, :], diff[1::4, :], diff[2::4, :],  diff[3::4, :]], 1)
    vertical_patterns = vertical_patterns.reshape(-1)
    vertical_patterns = vertical_patterns[:(dim ** 4) // 2]
    return np.concatenate([horizontal_patterns, vertical_patterns, [label]])
